...
!       dt=1d-17					! Time step in seconds (see below)
	D=125d-6					! Laser beam waist in meters
	zimp=24d-2+D					! Distance from laser to screen in meters
	v0=1.1d7					! Electron velocity
	E0L=2.6d8						! Laser electric field
        wL=3.54d15                                      ! Laser frequency
!       RESONANCE FREQUENCY FOR THE SHARP RESONANCE CONDITION <---- Herman says it must be the laser frequency
	wres=wL                                         ! Resonance frequency for the ZPF modes
!	wres=(9.10938356d-31)*((299792458d0)**2d0)/(1.0545718d-34) // wC ~ 7.8e20 rad/s

        Damping=6.245835d-24
!	PRINT*,"Damping=",Damping,"s"
        Delta=1d7*Damping*(wres**2d0)
        Nk=1000000 ! Number of zpf modes
        Ne=1 ! Number of polarisations
!       PRINT*,"Delta=",Delta,"1/s"
        dt=3.141592d0/(10d0*(wres+Delta/2d0))					! Time step in seconds
...
! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	CALL kernel_wrapper(init,pos,N,theta,phi,k,xi,eta,dt,D,zimp,v0,wL,wres,Delta,Nk,Ne,E0L) ! Without
